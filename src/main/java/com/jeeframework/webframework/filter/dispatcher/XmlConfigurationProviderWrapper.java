 ...
 */
public class XmlConfigurationProviderWrapper extends XmlConfigurationProvider {

    private static final Logger LOG = LoggerFactory.getLogger(XmlConfigurationProviderWrapper.class);
    ...
    public XmlConfigurationProviderWrapper() {
        this("xwork.xml", true);
    }

    public XmlConfigurationProviderWrapper(String filename) {
        this(filename, true);
    }

    public XmlConfigurationProviderWrapper(String filename, boolean errorIfMissing) {
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof XmlConfigurationProviderWrapper)) {
            return false;
        }

        final XmlConfigurationProviderWrapper XmlConfigurationProviderWrapper = (XmlConfigurationProviderWrapper) o;

        if ((configFileName != null) ? (!configFileName.equals(XmlConfigurationProviderWrapper.configFileName)) : (XmlConfigurationProviderWrapper.configFileName != null)) {
            return false;
        }
    ...
    protected void addAction(Element actionElement, PackageConfig.Builder packageContext) throws ConfigurationException {
        ...
        ActionConfig actionConfig = new ActionConfig.Builder(packageContext.getName(), name, className).methodName(methodName).addResultConfigs(results).addInterceptors(interceptorList)
                .addExceptionMappings(exceptionMappings).addParams(XmlHelper.getParams(actionElement)).addAllowedMethod(allowedMethods).location(location).build();
        packageContext.addActionConfig(name, actionConfig);

        if (LOG.isDebugEnabled()) {
            LOG.debug("Loaded " + (StringUtils.isNotEmpty(packageContext.getNamespace()) ? (packageContext.getNamespace() + "/") : "") + name + " in '" + packageContext.getName() + "' package:"
                    + actionConfig);
        }
    }
        ...
        if (StringUtils.isNotEmpty(StringUtils.defaultString(parent))) { // has parents, let's look it up
            ...
            for (String parentPackageName : ConfigurationUtil.buildParentListFromString(parent)) {
                if (configuration.getPackageConfigNames().contains(parentPackageName)) {
                    ...
                    throw new ConfigurationException(name + "     Parent package is not defined: " + parentPackageName);
                }
    ...
    protected Map<String, ResultConfig> buildResults(Element element, PackageConfig.Builder packageContext) {
        ...
        for (int i = 0; i < resultEls.getLength(); i++) {
            ...
            if (resultElement.getParentNode().equals(element) || resultElement.getParentNode().getNodeName().equals(element.getNodeName())) {
                ...
                if (config == null) {
                    throw new ConfigurationException("There is no result type defined for type '" + resultType + "' mapped with name '" + resultName + "'." + "  Did you mean '"
                            + guessResultType(resultType) + "'?", resultElement);
                }
                            ...
                            for (int j = 0; j < resultElement.getChildNodes().getLength(); j++) {
                                if (resultElement.getChildNodes().item(j).getNodeType() == Node.TEXT_NODE) {
                                    ...
                                    if (val != null) {
                                        ...
                                        if (resultType.equals("jsonjsp") && paramName.equals("location")) {
                                            ...
                                            String srcFilePath = webroot + val;
                                            String curFileName = com.jeeframework.util.string.StringUtils.getFileNamePreffix(val);
                                            String curFileSuffix = com.jeeframework.util.string.StringUtils.getFilenameExtension(val);

                                            String newSuffix = JSONJspServletDispatcherResult.JSONJSP_PREFIX;

                                            if (!(curFileName.startsWith("/") || curFileName.startsWith("\\"))) {
                                                newSuffix = newSuffix + "/";
                                            }

                                            newSuffix = newSuffix + curFileName + JSONJspServletDispatcherResult.JSONJSP_SUFFIX + "." + curFileSuffix;

                                            String dstFilePath = webroot + newSuffix;
                                            ...
                                            if (srcCurjsp.exists()) {
                                                try {
                                                    ...
                                                    e.printStackTrace();
                                                    System.out.println("��� jsonjsp " + srcCurjsp + "error����");
                                                }
                                            }
    ...
    protected List<ExceptionMappingConfig> buildExceptionMappings(Element element, PackageConfig.Builder packageContext) {
        ...
        for (int i = 0; i < exceptionMappingEls.getLength(); i++) {
            ...
            if (ehElement.getParentNode().equals(element) || ehElement.getParentNode().getNodeName().equals(element.getNodeName())) {
                ...
                ExceptionMappingConfig ehConfig = new ExceptionMappingConfig.Builder(emName, exceptionClassName, exceptionResult).addParams(params).location(DomHelper.getLocationObject(ehElement))
                        .build();
    ...
    protected Iterator<URL> getConfigurationUrls(String fileName) throws IOException {
        return ClassLoaderUtil.getResources(fileName, XmlConfigurationProviderWrapper.class, false);
    }
    ...
    /**
     * Looks up the Interceptor Class from the interceptor-ref name and creates an instance, which is added to the provided List, or, if this is a ref to a stack, it adds the Interceptor instances
     * from the List to this stack.
     * 
    ...
    public String toString() {
        return "XmlConfigurationProviderWrapper{" + "configFileName='" + configFileName + '\'' + '}';
    }
