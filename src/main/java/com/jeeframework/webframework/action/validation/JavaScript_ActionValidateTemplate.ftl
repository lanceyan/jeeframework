...
function validateForm_${actionAlias}(form,elementName) {
    var errors = false;
    <#list tag  as validator>
    // field name: ${validator.fieldName}
    // validator name: ${validator.validatorType}
    if (form.elements['${validator.fieldName}'] 
    	&& (elementName == null || elementName == undefined || elementName == '${validator.fieldName}')) {
        ...
        if (field.value != null) {
        	...
        	if (isNaN(field.value)) {
                ...
                if (<#if validator.min?exists>value <
                     ${validator.min}<#else>false</#if> ||
                        <#if validator.max?exists>value >
