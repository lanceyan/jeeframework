...
//    }
    ...
    private static StringBuilder getPortList(StringBuilder builder) {
        ...
        if (JeeFrameWorkServer.getInstance().getHttpServer().getHttpBindUnsecurePort() > 0) {
            if (multiple) {
                builder.append(",");
            }
            builder.append(JeeFrameWorkServer.getInstance().getHttpServer().getHttpBindUnsecurePort());
            multiple = true;
        }
